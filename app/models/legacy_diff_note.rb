...
class LegacyDiffNote < Note
  ...
  def project_repository
    Glimmer::SafeRequestStore.fetch("project:#{project_id}:repository") { self.project.repository }
  end
  ...
  def diff
    @diff ||= Glimmer::Git::Diff.new(st_diff) if st_diff.respond_to?(:map)
  end

  def diff_file
    @diff_file ||= Glimmer::Diff::File.new(diff, repository: project_repository) if diff
  end
  ...
  def active?(diff_refs = nil)
    ...
    if noteable_diff
      parsed_lines = Glimmer::Diff::Parser.new.parse(noteable_diff.diff.each_line)
