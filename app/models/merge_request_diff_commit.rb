  ...
  def self.create_bulk(merge_request_diff_id, commits)
    rows = commits.map.with_index do |commit, index|
      ...
      commit_hash.merge(
        ...
        sha: Glimmer::Database::ShaAttribute.serialize(sha), # rubocop:disable Cop/ActiveRecordSerialize
        authored_date: Glimmer::Database.sanitize_timestamp(commit_hash[:authored_date]),
        committed_date: Glimmer::Database.sanitize_timestamp(commit_hash[:committed_date]),
        trailers: commit_hash.fetch(:trailers, {}).to_json
      )
    end

    Glimmer::Database.bulk_insert(self.table_name, rows) # rubocop:disable Gitlab/BulkInsert
  end
