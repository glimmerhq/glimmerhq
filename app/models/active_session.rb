...
# Backing store for glimmer session data.
#
...
class ActiveSession
  ...
  def self.set(user, request)
    Glimmer::Redis::SharedState.with do |redis|
      ...
      redis.pipelined do
        redis.setex(
          ...
          Settings.glimmer['session_expire_delay'] * 60,
  ...
  def self.destroy_sessions(redis, user, session_ids)
    ...
    Glimmer::Instrumentation::RedisClusterValidator.allow_cross_slot_commands do
      redis.del(key_names)
  ...
  def self.rack_key_name(session_id)
    "#{Glimmer::Redis::SharedState::SESSION_NAMESPACE}:#{session_id}"
  end

  def self.key_name(user_id, session_id = '*')
    "#{Glimmer::Redis::SharedState::USER_SESSIONS_NAMESPACE}:#{user_id}:#{session_id}"
  end

  def self.lookup_key_name(user_id)
    "#{Glimmer::Redis::SharedState::USER_SESSIONS_LOOKUP_NAMESPACE}:#{user_id}"
  end
