...
class GroupGroupLink < ApplicationRecord
  ...
  validates :group_access, inclusion: { in: Glimmer::Access.all_values },
                           presence: true

  scope :non_guests, -> { where('group_access > ?', Glimmer::Access::GUEST) }
  ...
  def self.access_options
    Glimmer::Access.options_with_owner
  end

  def self.default_access
    Glimmer::Access::DEVELOPER
  end

  def human_access
    Glimmer::Access.human_access(self.group_access)
  end
end
