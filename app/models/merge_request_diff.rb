...
class MergeRequestDiff < ApplicationRecord
  ...
  class << self
    def ids_for_external_storage_migration(limit:)
      return [] unless Glimmer.config.external_diffs.enabled

      case Glimmer.config.external_diffs.when
      when 'always'
  ...
  def diffs_collection(diff_options = nil)
    Glimmer::Diff::FileCollection::MergeRequestDiff.new(self, diff_options: diff_options)
  end
  ...
  def migrate_files_to_external_storage!
    ...
    transaction do
      ...
      Glimmer::Database.bulk_insert('merge_request_diff_files', rows) # rubocop:disable Gitlab/BulkInsert
      save!
    end
  ...
  def diffs_in_batch_collection(batch_page, batch_size, diff_options:)
    Glimmer::Diff::FileCollection::MergeRequestDiffBatch.new(self,
                                                            batch_page,
                                                            batch_size,
  ...
  def use_external_diff?
    return false unless Glimmer.config.external_diffs.enabled
  ...
  def load_diffs(options)
    ...
    opening_external_diff do
      ...
      Glimmer::Git::DiffCollection.new(collection.map(&:to_hash), options)
    end
  end
  ...
  def sort_diffs(diffs)
    ...
    Glimmer::Diff::FileCollectionSorter.new(diffs).sort
  end
