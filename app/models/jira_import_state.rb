...
class JiraImportState < ApplicationRecord
  ...
  state_machine :status, initial: :initial do
    ...
    after_transition initial: :scheduled do |state, _|
      state.run_after_commit do
        job_id = Glimmer::JiraImport::Stage::StartImportWorker.perform_async(project.id)
  ...
  def store_issue_counts
    import_label_id = Glimmer::JiraImport.get_import_label_id(project.id)

    failed_to_import_count = Glimmer::JiraImport.issue_failures(project.id)
  ...
  def mark_as_failed(error_message)
    sanitized_message = Glimmer::UrlSanitizer.sanitize(error_message)

    do_fail(error_message: error_message)
  rescue ActiveRecord::ActiveRecordError => e
    Glimmer::AppLogger.error("Error setting import status to failed: #{e.message}. Original error: #{sanitized_message}")
  end
