...
class Commit
  ...
  extend Glimmer::Cache::RequestCache
  ...
  MIN_SHA_LENGTH = Glimmer::Git::Commit::MIN_SHA_LENGTH
  ...
  class << self
    ...
    def diff_line_count(diffs)
      diffs.reduce(0) { |sum, d| sum + Glimmer::Git::Util.count_lines(d.diff) }
    end
    ...
    def diff_safe_lines(project: nil)
      Glimmer::Git::DiffCollection.default_limits(project: project)[:max_lines]
    end
    ...
    def from_hash(hash, container)
      raw_commit = Glimmer::Git::Commit.new(container.repository.raw, hash)
  ...
  def hook_attrs(with_changed_files: false)
    data = {
      ...
      title: title,
      timestamp: timestamp,
      url: Glimmer::UrlBuilder.build(self),
      author: {
        name: author_name,
        email: author_email
      }
  ...
  def diff_refs
    Glimmer::Diff::DiffRefs.new(
      base_sha: self.parent_id || Glimmer::Git::BLANK_SHA,
      head_sha: self.sha
    )
  end
  ...
  def raw_signature_type
    strong_memoize(:raw_signature_type) do
      next unless @raw.instance_of?(Glimmer::Git::Commit)
  ...
  def signature
    strong_memoize(:signature) do
      ...
      when :X509
        Glimmer::X509::Commit.new(self).signature
      else
        nil
      end
    end
  ...
  def gpg_commit
    @gpg_commit ||= Glimmer::Gpg::Commit.new(self)
  end
  ...
  def uri_type(path)
    ...
    if entry[:type] == :blob
      blob = ::Blob.decorate(Glimmer::Git::Blob.new(name: entry[:name]), container)
  ...
  def diffs(diff_options = {})
    Glimmer::Diff::FileCollection::Commit.new(self, diff_options: diff_options)
  end
  ...
  DRAFT_REGEX = /\A\s*#{Regexp.union(Glimmer::Regex.merge_request_wip, Glimmer::Regex.merge_request_draft)}|(fixup!|squash!)\s/.freeze
