...
class Issue < ApplicationRecord
  ...
  scope :order_due_date_asc, -> { reorder(::Glimmer::Database.nulls_last_order('due_date', 'ASC')) }
  scope :order_due_date_desc, -> { reorder(::Glimmer::Database.nulls_last_order('due_date', 'DESC')) }
  scope :order_closest_future_date, -> { reorder(Arel.sql('CASE WHEN issues.due_date >= CURRENT_DATE THEN 0 ELSE 1 END ASC, ABS(CURRENT_DATE - issues.due_date) ASC')) }
  scope :order_relative_position_asc, -> { reorder(::Glimmer::Database.nulls_last_order('relative_position', 'ASC')) }
  ...
  def self.order_by_position_and_priority(with_cte: false)
    order_labels_priority(with_cte: with_cte)
      .reorder(Glimmer::Database.nulls_last_order('relative_position', 'ASC'),
              Glimmer::Database.nulls_last_order('highest_priority', 'ASC'),
              "id DESC")
  end

  def hook_attrs
    Glimmer::HookData::IssueBuilder.new(self).build
  end
  ...
  def visible_to_user?(user = nil)
    ...
    user.can_read_all_resources? ||
      ::Glimmer::ExternalAuthorization.access_allowed?(
  ...
  def record_create_action
    Glimmer::UsageDataCounters::IssueActivityUniqueCounter.track_issue_created_action(author: author)
  end
  ...
  def readable_by?(user)
    if user.can_read_all_resources?
      ...
      project.team.member?(user, Glimmer::Access::REPORTER)
    else
      project.public? ||
  ...
  def publicly_visible?
    project.public? && !confidential? && !::Glimmer::ExternalAuthorization.enabled?
  end

  def expire_etag_cache
    key = Glimmer::Routing.url_helpers.realtime_changes_project_issue_path(project, self)
