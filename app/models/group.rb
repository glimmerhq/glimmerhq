...
class Group < Namespace
  ...
  has_many :group_members, -> { where(requested_at: nil).where.not(members: { access_level: Glimmer::Access::MINIMAL_ACCESS }) }, dependent: :destroy, as: :source # rubocop:disable Cop/ActiveRecordDependent
  alias_method :members, :group_members

  has_many :users, through: :group_members
  has_many :owners,
    -> { where(members: { access_level: Glimmer::Access::OWNER }) },
  ...
  validates :name,
            html_safety: true,
            format: { with: Glimmer::Regex.group_name_regex,
                      message: Glimmer::Regex.group_name_regex_message },
            if: :name_changed?
  ...
  scope :for_authorized_group_members, -> (user_ids) do
    joins(:group_members)
      ...
      .where("access_level >= ?", Glimmer::Access::GUEST)
  end
  ...
  class << self
    ...
    def public_to_user_arel(user)
      self.arel_table[:visibility_level]
        .in(Glimmer::VisibilityLevel.levels_for_user(user))
    end
  ...
  def packages_feature_enabled?
    ::Glimmer.config.packages.enabled
  end

  def dependency_proxy_feature_available?
    ::Glimmer.config.dependency_proxy.enabled
  end
  ...
  def web_url(only_path: nil)
    Glimmer::UrlBuilder.build(self, only_path: only_path)
  end
  ...
  def lfs_enabled?
    return false unless Glimmer.config.lfs.enabled
    return Glimmer.config.lfs.enabled if self[:lfs_enabled].nil?
  ...
  def last_blocked_owner?(user)
    ...
    blocked_owners = members.blocked.where(access_level: Glimmer::Access::OWNER)
  ...
  def post_create_hook
    Glimmer::AppLogger.info("Group \"#{name}\" was created")

    system_hook_service.execute_hooks_for(self, :create)
  end

  def post_destroy_hook
    Glimmer::AppLogger.info("Group \"#{name}\" was removed")
  ...
  def ci_variables_for(ref, project, environment: nil)
    ...
    ::Glimmer::SafeRequestStore.fetch(cache_key) do
  ...
  def project_creation_level
    super || ::Glimmer::CurrentSettings.default_project_creation
  end

  def subgroup_creation_level
    super || ::Glimmer::Access::OWNER_SUBGROUP_ACCESS
  end
  ...
  def has_project_with_service_desk_enabled?
    Glimmer::ServiceDesk.supported? && all_projects.service_desk_enabled.exists?
  end
