...
class Key < ApplicationRecord
  ...
  scope :order_last_used_at_desc, -> { reorder(::Glimmer::Database.nulls_last_order('last_used_at', 'DESC')) }
  ...
  def key=(value)
    write_attribute(:key, value.present? ? Glimmer::SSHPublicKey.sanitize(value) : nil)
  ...
  def publishable_key
    ...
    self.key.split[0..1].push("#{self.user_name} (#{Glimmer.config.glimmer.host})").join(' ')
  end
  ...
  def add_to_authorized_keys
    return unless Glimmer::CurrentSettings.authorized_keys_enabled?
  ...
  def public_key
    @public_key ||= Glimmer::SSHPublicKey.new(key)
  end
  ...
  def key_meets_restrictions
    restriction = Glimmer::CurrentSettings.key_restriction_for(public_key.type)
  ...
  def forbidden_key_type_message
    allowed_types = Glimmer::CurrentSettings.allowed_key_types.map(&:upcase)

    "type is forbidden. Must be #{Glimmer::Utils.to_exclusive_sentence(allowed_types)}"
  end
end
