...
class MergeRequest < ApplicationRecord
  ...
  include Glimmer::Utils::StrongMemoize
  ...
  ALLOWED_TO_USE_MERGE_BASE_PIPELINE_FOR_COMPARISON = {
    'Ci::CompareCodequalityReportsService' => ->(project) { ::Glimmer::Ci::Features.display_codequality_backend_comparison?(project) }
  }.freeze
  ...
  state_machine :merge_status, initial: :unchecked do
    ...
    around_transition do |merge_request, transition, block|
      Glimmer::Timeless.timeless(merge_request, &block)
    end
  ...
  scope :order_merged_at, ->(direction) do
    ...
    order = Glimmer::Pagination::Keyset::Order.build([
      Glimmer::Pagination::Keyset::ColumnOrderDefinition.new(
        attribute_name: 'merge_request_metrics_merged_at',
        column_expression: MergeRequest::Metrics.arel_table[:merged_at],
        order_expression: Glimmer::Database.nulls_last_order('merge_request_metrics.merged_at', direction),
        reversed_order_expression: Glimmer::Database.nulls_first_order('merge_request_metrics.merged_at', reversed_direction),
  ...
  def rebase_in_progress?
    rebase_jid.present? && Glimmer::SidekiqStatus.running?(rebase_jid)
  end
  ...
  DRAFT_REGEX = /\A*#{Regexp.union(Glimmer::Regex.merge_request_wip, Glimmer::Regex.merge_request_draft)}+\s*/i.freeze
  ...
  def hook_attrs
    Glimmer::HookData::MergeRequestBuilder.new(self).build
  end
  ...
  def note_positions_for_paths(paths, user = nil)
    ...
    collection = Glimmer::Diff::PositionCollection.new(positions, diff_head_sha)
  ...
  def discussions_diffs
    strong_memoize(:discussions_diffs) do
      ...
      Glimmer::DiscussionsDiff::FileCollection.new(note_diff_files.to_a)
    end
  end
  ...
  def source_branch_ref
    ...
    Glimmer::Git::BRANCH_REF_PREFIX + source_branch
  end
  ...
  def target_branch_ref
    ...
    Glimmer::Git::BRANCH_REF_PREFIX + target_branch
  end
  ...
  def validate_branch_name(attr)
    ...
    errors.add(attr) unless Glimmer::GitRefValidator.validate_merge_request_branch(branch)
  end
  ...
  def merge_ongoing?
    ...
    !!merge_jid && !merged? && Glimmer::SidekiqStatus.running?(merge_jid)
  end
  ...
  def create_merge_request_diff
    ...
    Glimmer::GitalyClient.allow_n_plus_1_calls do
  ...
  def merge_request_diff_for(diff_refs_or_sha)
    matcher =
      if diff_refs_or_sha.is_a?(Glimmer::Diff::DiffRefs)
        {
  ...
  def should_remove_source_branch?
    Glimmer::Utils.to_boolean(merge_params['should_remove_source_branch'])
  end

  def force_remove_source_branch?
    Glimmer::Utils.to_boolean(merge_params['force_remove_source_branch'])
  end
  ...
  def closes_issues(current_user = self.author)
    if target_branch == project.default_branch
      ...
      Glimmer::ClosingIssueExtractor.new(project, current_user)
  ...
  def issues_mentioned_but_not_closing(current_user)
    ...
    ext = Glimmer::ReferenceExtractor.new(project, current_user)
    ext.analyze("#{title}\n#{description}")
  ...
  def can_be_merged_by?(user, skip_collaboration_check: false)
    access = ::Glimmer::UserAccess.new(user, container: project, skip_collaboration_check: skip_collaboration_check)
    access.can_update_branch?(target_branch)
  end

  def can_be_merged_via_command_line_by?(user)
    access = ::Glimmer::UserAccess.new(user, container: project)
  ...
  def predefined_variables
    Glimmer::Ci::Variables::Collection.new.tap do |variables|
  ...
  def has_codequality_mr_diff_report?
    return false unless ::Glimmer::Ci::Features.display_quality_on_mr_diff?(project)
  ...
  def has_codequality_reports?
    return false unless ::Glimmer::Ci::Features.display_codequality_backend_comparison?(project)
  ...
  def first_contribution?
    return false if project.team.max_member_access(author_id) > Glimmer::Access::GUEST
  ...
  def collaborative_push_possible?
    source_project.present? && for_fork? &&
      target_project.visibility_level > Glimmer::VisibilityLevel::PRIVATE &&
      source_project.visibility_level > Glimmer::VisibilityLevel::PRIVATE &&
      !ProtectedBranch.protected?(source_project, source_branch)
  end
  ...
  def with_retried_nowait_lock
    ...
    Glimmer::ErrorTracking.track_exception(e)
  ...
  def expire_etag_cache
    ...
    key = Glimmer::Routing.url_helpers.cached_widget_project_json_merge_request_path(project, self, format: :json)
    Glimmer::EtagCaching::Store.new.touch(key)
  end
