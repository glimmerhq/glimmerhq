...
class Environment < ApplicationRecord
  ...
  validates :name,
            ...
            length: { maximum: 255 },
            format: { with: Glimmer::Regex.environment_name_regex,
                      message: Glimmer::Regex.environment_name_regex_message }

  validates :slug,
            presence: true,
            uniqueness: { scope: :project_id },
            length: { maximum: 24 },
            format: { with: Glimmer::Regex.environment_slug_regex,
                      message: Glimmer::Regex.environment_slug_regex_message }
  ...
  scope :order_by_last_deployed_at, -> do
    order(Glimmer::Database.nulls_first_order("(#{max_deployment_id_sql})", 'ASC'))
  end
  scope :order_by_last_deployed_at_desc, -> do
    order(Glimmer::Database.nulls_last_order("(#{max_deployment_id_sql})", 'DESC'))
  end
  ...
  # Do not set a large limit unless you've confirmed that it works on glimmerhq.com scale.
  ...
  class << self
    ...
    def cte_for_deployments_with_stop_action
      Glimmer::SQL::CTE.new(:deployments_with_stop_action,
  ...
  def cluster_prometheus_adapter
    @cluster_prometheus_adapter ||= ::Glimmer::Prometheus::Adapter.new(project, deployment_platform&.cluster).cluster_prometheus_adapter
  end

  def predefined_variables
    Glimmer::Ci::Variables::Collection.new
  ...
  def cancel_deployment_jobs!
    ...
    jobs.each do |deployment|
      Glimmer::OptimisticLocking.retry_lock(deployment.deployable, name: 'environment_cancel_deployment_jobs') do |deployable|
        deployable.cancel! if deployable&.cancelable?
      end
    rescue => e
      Glimmer::ErrorTracking.track_exception(e, environment_id: id, deployment_id: deployment.id)
    end
  end
  ...
  def prometheus_adapter
    @prometheus_adapter ||= Glimmer::Prometheus::Adapter.new(project, deployment_platform&.cluster).prometheus_adapter
  end
  ...
  def expire_etag_cache
    Glimmer::EtagCaching::Store.new.tap do |store|
      store.touch(etag_cache_key)
    end
  end

  def etag_cache_key
    Glimmer::Routing.url_helpers.project_environments_path(
      project,
      format: :json)
  end
  ...
  def rollout_status
    ...
    result || ::Glimmer::Kubernetes::RolloutStatus.loading
  end
  ...
  def generate_slug
    self.slug = Glimmer::Slug::Environment.new(name).generate
  end
