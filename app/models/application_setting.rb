...
class ApplicationSetting < ApplicationRecord
  ...
  validates :max_pages_size,
            presence: true,
            numericality: { only_integer: true, greater_than_or_equal_to: 0,
                            less_than: ::Glimmer::Pages::MAX_SIZE / 1.megabyte }
  ...
  validates :gitaly_timeout_default,
            ...
            numericality: {
              ...
              less_than_or_equal_to: Settings.glimmer.max_request_duration_seconds
            }
  ...
  validates :diff_max_patch_bytes,
            presence: true,
            numericality: { only_integer: true,
                            greater_than_or_equal_to: Glimmer::Git::Diff::DEFAULT_MAX_PATCH_BYTES,
                            less_than_or_equal_to: Glimmer::Git::Diff::MAX_PATCH_BYTES_UPPER_BOUND }
  ...
  validates_each :restricted_visibility_levels do |record, attr, value|
    value&.each do |level|
      unless Glimmer::VisibilityLevel.options.value?(level)
  ...
  validates_each :import_sources do |record, attr, value|
    value&.each do |source|
      unless Glimmer::ImportSources.options.value?(source)
  ...
  validates :eks_account_id,
            format: { with: Glimmer::Regex.aws_account_id_regex,
                      message: Glimmer::Regex.aws_account_id_message },
  ...
  def self.cache_backend
    Glimmer::ProcessMemoryCache.cache_backend
  end
  ...
  kroki_formats_attributes.keys.each do |key|
    define_method :"kroki_formats_#{key}=" do |value|
      super(::Glimmer::Utils.to_boolean(value))
    end
  end
  ...
  def kroki_format_supported?(diagram_type)
    ...
    return kroki_formats_blockdiag if ::Glimmer::Kroki::BLOCKDIAG_FORMATS.include?(diagram_type)
  ...
  def parsed_grafana_url
    @parsed_grafana_url ||= Glimmer::Utils.parse_url(grafana_url)
  end

  def parsed_kroki_url
    @parsed_kroki_url ||= Glimmer::UrlBlocker.validate!(kroki_url, schemes: %w(http https), enforce_sanitization: true)[0]
  rescue Glimmer::UrlBlocker::BlockedUrlError => error
    self.errors.add(
      :kroki_url,
      "is not valid. #{error}"
    )
