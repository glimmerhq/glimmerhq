...
class GpgKey < ApplicationRecord
  ...
  def extract_fingerprint
    ...
    # only allows one key
    self.fingerprint = Glimmer::Gpg.fingerprints_from_key(key).first
  end

  def extract_primary_keyid
    # we can assume that the result only contains one item as the validation
    # only allows one key
    self.primary_keyid = Glimmer::Gpg.primary_keyids_from_key(key).first
  end

  def generate_subkeys
    gpg_subkeys = Glimmer::Gpg.subkeys_from_key(key)
