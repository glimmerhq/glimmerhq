...
namespace :gettext do
  ...
  task :regenerate do
    ...
    unless system(*%w(git -c core.hooksPath=/dev/null checkout -- locale/*/glimmer.po))
      raise 'failed to cleanup generated locale/*/glimmer.po files'
    end

    raise 'glimmer.pot file not generated' unless File.exist?(pot_file_path)
    ...
    puts <<~MSG
    All done. Please commit the changes to `locale/glimmer.pot`.
  ...
  task lint: :environment do
    ...
    files = Dir.glob(Rails.root.join('locale/*/glimmer.po'))

    linters = files.map do |file|
      locale = File.basename(File.dirname(file))

      Glimmer::I18n::PoLinter.new(po_path: file, locale: locale)
    end

    linters.unshift(Glimmer::I18n::PoLinter.new(po_path: pot_file_path))
  ...
  task :updated_check do
    ...
    # Having `LC_MESSAGES/glimmer.mo files present also confuses the output.
    FileUtils.rm Dir['locale/**/glimmer.*']
    FileUtils.rm_f pot_file_path
  ...
  def html_todolist
    ...
    @html_todolist = YAML.safe_load(File.read(Rails.root.join('lib/glimmer/i18n/html_todo.yml')))
  end
  ...
  def ensure_locale_folder_presence!
    unless Dir.exist?(locale_path)
      raise <<~MSG
      Cannot find '#{locale_path}' folder. Please ensure you're running this task from the glimmer repo.
  ...
  def pot_file_path
    @pot_file_path ||= File.join(locale_path, 'glimmer.pot')
  end
end
