...
namespace :gemojione do
  ...
  task aliases: ['yarn:check', 'environment'] do
    ...
    index = Glimmer::Json.parse(File.read(index_file))
    ...
    File.open(out, 'w') do |handle|
      handle.write(Glimmer::Json.pretty_generate(aliases, indent: '   ', space: '', space_before: ''))
    end
  end
  ...
  task digests: ['yarn:check', 'environment'] do
    ...
    # We don't have `node_modules` available in built versions of glimmer
    ...
    Glimmer::Emoji.emojis.each do |name, emoji_hash|
      # Ignore aliases
      unless Glimmer::Emoji.emojis_aliases.key?(name)
        ...
        entry = {
          ...
          unicodeVersion: Glimmer::Emoji.emoji_unicode_version(name),
          digest: hash_digest
        }
        ...
        new_entry = {
          ...
          u: Glimmer::Emoji.emoji_unicode_version(name)
        }
    ...
    File.open(out, 'w') do |handle|
      handle.write(Glimmer::Json.pretty_generate(resultant_emoji_map))
    end

    out_new = File.join(Rails.root, 'public', '-', 'emojis', '1', 'emojis.json')
    File.open(out_new, 'w') do |handle|
      handle.write(Glimmer::Json.pretty_generate(resultant_emoji_map_new))
    end
  end
