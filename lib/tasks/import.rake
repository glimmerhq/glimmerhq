...
class GithubImport
  ...
  def initialize(token, glimmer_username, project_path, extras)
    @options = { token: token }
    @project_path = project_path
    @current_user = UserFinder.new(glimmer_username).find_by_username

    raise "glimmer user #{glimmer_username} not found. Please specify a valid username." unless @current_user
  ...
  def show_warning!
    puts "This will import GitHub #{@repo.full_name.bright} into glimmer #{@project_path.bright} as #{@current_user.name}"
  ...
  def import!
    ...
    timings = Benchmark.measure do
      import_success = Glimmer::GithubImport::SequentialImporter
  ...
  def visibility_level
    @repo.private ? Glimmer::VisibilityLevel::PRIVATE : Glimmer::CurrentSettings.current_application_settings.default_project_visibility
  end
end

class GithubRepos
  def initialize(token, current_user, github_repo)
    @client = Glimmer::GithubImport::Client.new(token)
...
namespace :import do
  desc 'glimmer | Import | Import a GitHub project - Example: import:github[ToKeN,root,root/blah,my/github_repo] (optional my/github_repo)'
  task :github, [:token, :glimmer_username, :project_path] => :environment do |_t, args|
    abort 'Project path must be: namespace(s)/project_name'.color(:red) unless args.project_path.include?('/')

    GithubImport.run!(args.token, args.glimmer_username, args.project_path, args.extras)
  end
end
