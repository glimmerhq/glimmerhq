...
task schema_version_check: :environment do
  ...
  if schema_version > 0 && schema_version < Glimmer::Database::MIN_SCHEMA_VERSION
    raise "Your current database version is too old to be migrated. " \
          "You should upgrade to glimmer #{Glimmer::Database::MIN_SCHEMA_GLIMMER_VERSION} before moving to this version. " \
