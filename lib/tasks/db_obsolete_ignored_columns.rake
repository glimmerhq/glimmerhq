...
task 'db:obsolete_ignored_columns' => :environment do
  list = Glimmer::Database::ObsoleteIgnoredColumns.new.execute
  ...
  if list.empty?
    ...
    puts <<~TEXT
      ...
      WARNING: Removing columns is tricky because running glimmer processes may still be using the columns.
